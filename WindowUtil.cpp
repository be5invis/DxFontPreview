...
long WindowPosition::GetWidth() const {
    return rect.right - rect.left;
}

void WindowPosition::SetWidth(long width) {
    rect.right = rect.left + width;
}


long WindowPosition::GetHeight() const {
    return rect.bottom - rect.top;
}

void WindowPosition::SetHeight(long height) {
...
void WindowPosition::Update(
    ...
    for (uint32_t i = 0; i < windowPositionsCount; ++i) {
        InvalidateRect(windowPositions[i].hwnd, NULL, true);
    }
}
...
void WinUtil::AmendComboBoxItemWidth(HWND box) {
    RECT rect;
    GetWindowRect(box, &rect);
    SendMessage(box, CB_SETDROPPEDWIDTH, WPARAM(2 * (rect.right - rect.left)), 0);
}

std::wstring WinUtil::GetValueOfComboBox(HWND box) {
    if (box) {
        int ItemIndex = SendMessage(box, CB_GETCURSEL, (WPARAM)0, (LPARAM)0);
        size_t textLength = SendMessage(box, CB_GETLBTEXTLEN, ItemIndex, 0);
        std::vector<WCHAR> recv(1 + textLength);
        SendMessage(box, CB_GETLBTEXT, ItemIndex, (LPARAM)&recv[0]);
        std::wstring currentContent(&recv[0]);
...
std::wstring WinUtil::GetValueOfTextBox(HWND box) {
    if (box) {
        size_t textLength = GetWindowTextLengthW(box);
        std::vector<WCHAR> recv(1 + textLength);
        GetWindowTextW(box, &recv[0], 1 + textLength);
        std::wstring currentContent(&recv[0]);
        return currentContent;
    } else {
        return std::wstring();
...
float WinUtil::GetNumOfComboBox(HWND box, const float& defaultValue) {
    std::wstring rep = GetValueOfComboBox(box);
    try {
        return std::stof(rep);
    } catch (...) {
        return defaultValue;
...
float WinUtil::GetNumOfTextBox(HWND box, const float& defaultValue) {
    std::wstring rep = GetValueOfTextBox(box);
    try {
        return std::stof(rep);
    } catch (...) {
        return defaultValue;
...
void WinUtil::SetComboBoxItem(HWND box, const std::wstring& str) {
    LRESULT matchIndex = SendMessage(box, CB_FINDSTRING, -1, LPARAM(str.c_str()));
    if (matchIndex != CB_ERR) {
        SendMessage(box, CB_SETCURSEL, WPARAM(matchIndex), 0);
    } else {
        SendMessage(box, WM_SETTEXT, 0, LPARAM(str.c_str()));
    }
}

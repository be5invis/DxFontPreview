...
namespace WinUtil {
    void AmendComboBoxItemWidth(HWND box);

    std::wstring GetValueOfComboBox(HWND box);
    std::wstring GetValueOfTextBox(HWND box);

    float GetNumOfComboBox(HWND box, const float& defaultValue);
    float GetNumOfTextBox(HWND box, const float& defaultValue);

    void SetComboBoxItem(HWND box, const std::wstring& str);
}